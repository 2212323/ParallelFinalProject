...
int main() {
  ...
  if (indexFile.is_open()) {
    ...
    while (!indexFile.eof()) {
      ...
      bitset<MAX_SIZE / BLOCK_SIZE> *secondaryIndex =
  ...
  vector<vector<unsigned int>> query_Lengths_searched(
      1000); // 有1000组数据（需随数据集修改）
  ...
  for (size_t i = 0; i < secondaryqueryDataSize; i++) {
    ...
    for (size_t j = 0; j < query_Lengths_searched[i].size();
         j++) // queryData[i].size()为每组列表的个数
    {
